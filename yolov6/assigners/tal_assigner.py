...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self,
        ...
        """
        # image = cv2.imread('data/coco128/images/train2017/000000000030.jpg')
        # def draw_anchor_boxes(image, pd_bboxes):
        #     # Convert the image to RGB color space
        #     image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        #     # Iterate over each anchor box
        #     a = pd_bboxes.cpu().numpy()
        #     for box in a[0]:
        ...
        #     cv2.waitKey(0)
        #     cv2.destroyAllWindows()
        # draw_anchor_boxes(image, pd_bboxes)
        ...
        for i in range(cycle):
            ...
            pd_scores_ = pd_scores[start:end, ...] # 1,21504,1
            pd_bboxes_ = pd_bboxes[start:end, ...] # 1,21504,4
            gt_labels_ = gt_labels[start:end, ...] # 1,297,1
            gt_bboxes_ = gt_bboxes[start:end, ...] # 1,297,4
            mask_gt_   = mask_gt[start:end, ...] # 1,297,1

            mask_pos, align_metric, overlaps = self.get_pos_mask(
                pd_scores_, pd_bboxes_, gt_labels_, gt_bboxes_, anc_points, mask_gt_) # positive mask for the predicted bounding boxes based on the matching between predicted and ground truth bounding boxes

            target_gt_idx, fg_mask, mask_pos = select_highest_overlaps(
                mask_pos, overlaps, self.n_max_boxes) # if an anchor box is assigned to multiple gts,
        ...
        # concat
        target_labels = torch.cat(target_labels_lst, 0) # 2 64512
        target_bboxes = torch.cat(target_bboxes_lst, 0) # 2 64512 4
        target_scores = torch.cat(target_scores_lst, 0) # 2 64512 1
    ...
    def get_pos_mask(self,
        ...
        align_metric, overlaps = self.get_box_metrics(pd_scores, pd_bboxes, gt_labels, gt_bboxes) # classes and IOUs # each ground truth(scores and label)
        # compared with 21504 predicted scores to calculate the best prediction
        # get in_gts mask
        mask_in_gts = select_candidates_in_gts(anc_points, gt_bboxes) #  used to select positive anchor centers that fall inside the ground truth bounding boxes.
    ...
    def get_box_metrics(self,
                        ...
                        gt_labels,
                        gt_bboxes):
        pd_scores = pd_scores.permute(0, 2, 1) # dimension rearrangement
        gt_labels = gt_labels.to(torch.long) # datatype conversion for operations that require integers, such as indexing or comparison operations.
        ind = torch.zeros([2, self.bs, self.n_max_boxes], dtype=torch.long)
        ind[0] = torch.arange(end=self.bs).view(-1, 1).repeat(1, self.n_max_boxes)
        ind[1] = gt_labels.squeeze(-1)
        bbox_scores = pd_scores[ind[0], ind[1]]

        overlaps = iou_calculator(gt_bboxes, pd_bboxes) # (1,297,4) (1,21504,4)
