...
class Detect(nn.Module):
    ...
    def __init__(self, num_classes=80, anchors=None, num_layers=3, inplace=True, head_layers=None, use_dfl=True, reg_max=16):  # detection layer
        ...
        self.use_dfl = use_dfl # Consider Decoupled Foreground Loss or not?
        self.reg_max = reg_max # represents the maximum value for bounding box regression. 
    ...
    def forward(self, x):
        if self.training:
            ...
            for i in range(self.nl):
                ...
                cls_x = x[i] # 2,64,80,80 # 2,128,40,40 # 2,256,20,20
                reg_x = x[i] # 2,64,80,80 # 2,128,40,40 # 2,256,20,20

                cls_feat = self.cls_convs[i](cls_x) # 2,64,80,80 # 2,128,40,40 # # 2,256,20,20
                reg_feat = self.reg_convs[i](reg_x) # 2,64,80,80 # 2,128,40,40 # # 2,256,20,20

                #anchor_base
                cls_output_ab = self.cls_preds_ab[i](cls_feat) # 2, 240, 80, 80 # 2,240,40,40 # # 2,240,20,20
                reg_output_ab = self.reg_preds_ab[i](reg_feat) # 2, 12, 80, 80 # 2,12,40,40 # 2, 12 ,20, 20

                cls_output_ab = torch.sigmoid(cls_output_ab)
                cls_output_ab = cls_output_ab.reshape(b, self.na, -1, h, w).permute(0,1,3,4,2) # 2,3,80,80,80 # 2,3,40,40,80
                cls_score_list_ab.append(cls_output_ab.flatten(1,3)) # 2,19200,80 # 2,4800,80
                ...
                # 2,3,80,80,4 # 2,3,40,40,4
                reg_dist_list_ab.append(reg_output_ab.flatten(1,3)) # 2, 19200, 4 # 2, 4800, 4

                #anchor_free
                cls_output_af = self.cls_preds_af[i](cls_feat) # 2,80,80,80 # 2,80,40,40
                reg_output_af = self.reg_preds_af[i](reg_feat) # 2, 4, 80,80 # 2,4,40,40

                cls_output_af = torch.sigmoid(cls_output_af)
                cls_score_list_af.append(cls_output_af.flatten(2).permute((0, 2, 1))) # 2,6400,80 # 2,1600,80  
                reg_dist_list_af.append(reg_output_af.flatten(2).permute((0, 2, 1))) # 2,6400,4 # 2,1600,4
                
            
            cls_score_list_ab = torch.cat(cls_score_list_ab, axis=1) # 2,25200,80
            reg_dist_list_ab = torch.cat(reg_dist_list_ab, axis=1) # 2,25200, 4
            cls_score_list_af = torch.cat(cls_score_list_af, axis=1) # 2, 8400, 80
            reg_dist_list_af = torch.cat(reg_dist_list_af, axis=1) # 2, 8400, 4
